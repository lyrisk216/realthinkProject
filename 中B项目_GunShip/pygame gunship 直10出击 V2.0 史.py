...
class CLS_bullet(object):
    def __init__(self, picFile, x, y, w, h, spdX, spdY):
        pic = pygame.image.load(picFile)
        pic.set_colorkey( (192, 192, 192) )
        self.pic = pic
        pic2 = pygame.image.load('bang.bmp')
        self.pic2 = pic2
        self.x, self.y, self.w, self.h = x, y, w, h
        self.interval, self.frameNum = 4, 4
        self.counter = 0
    ...
    def move(self):
        ...
        if self.y < SPACE_UP:
            self.y = SPACE_UP
        if self.y > SPACE_DOWN - self.h:
            self.x += - self.spdX - fwork.z10.spdX
...
class CLS_framework(object):               
    ...
    def keydown(self, key):
        ...
        if event.key == pygame.K_RETURN:
            ...
            self.z10.bulletList = []
        if event.key == ord(' '):
            bullet1 = CLS_bullet('shit.bmp', self.z10.x + 62, self.z10.y + 26, 48, 48, 5, -2)
            bullet2 = CLS_bullet('shit.bmp', self.z10.x + 62, self.z10.y + 26, 48, 48, 5, 0)
            bullet3 = CLS_bullet('shit.bmp', self.z10.x + 62, self.z10.y + 26, 48, 48, 5, 2)
            
            self.z10.bulletList.append(bullet1)
            self.z10.bulletList.append(bullet2)
            self.z10.bulletList.append(bullet3)
            self.z10.bulletSound.play()
    def keyup(self, key):
        if event.key == pygame.K_UP:
            self.z10.accY = 0
#------------------------------
